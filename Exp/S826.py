...
# For Windows: use .dll file
# For Linux: use .so file
# s826dll = cdll.LoadLibrary(os.path.join(current_dir, "sdk_826_win_3.3.9/s826_3.3.9/api/x64/s826.dll"))  # Windows
s826dll = cdll.LoadLibrary(os.path.join(current_dir, "sdk_826_linux_3.3.16/sdk_826_linux_3.3.16/middleware/lib826_64.so"))  # Linux
BOARD = 0
...
class S826(object):
    ...
    def setpoint(self, B_x, B_y, B_z):
        setpoint_x = int(32768 + 3276.8 *3 * 1 * B_x / 10)
        setpoint_y = int(32768 + 3276.8 *3 * 1 * B_y / 10)
        setpoint_z = int(32768 + 3276.8 *3 * B_z / 10)

        s826dll.S826_DacDataWrite(BOARD, 3, setpoint_x, 0)
        s826dll.S826_DacDataWrite(BOARD, 6, setpoint_x, 0)
        s826dll.S826_DacDataWrite(BOARD, 4, setpoint_y, 0)
        s826dll.S826_DacDataWrite(BOARD, 5, setpoint_z, 0)
...
if __name__ == '__main__':
    ...
    try:
        while True:
            coil.rotating_field(flux_density=10, frequency=10, pitch=00, direction=90)
